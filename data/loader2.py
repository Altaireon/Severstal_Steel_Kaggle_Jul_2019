...
#            collate_fn  = null_collate
    ...
    def run_check_loader(self):
        ...
        for img,mask in self.train_loader:
            ...
            assert(mask.shape == (self.params['batch_size'][0],self.params['preprocess']['input_size'][0],self.params['preprocess']['input_size'][1],self.params['num_class']))
            break
#        itr = 0
...
#        assert(itr == self.train_loader.dataset.df.shape[0])
        ...
        for img,mask in self.valid_loader:
            self.logger.info(f'Image Shape = {img.shape}')
            assert(img.shape == (self.params['batch_size'][1],3,256,1600))
            self.logger.info(f'Mask Shape = {mask.shape}')
            assert(mask.shape == (self.params['batch_size'][1],256,1600,self.params['num_class']))
            break
#        itr = 0
...
#        assert(itr == self.valid_loader.dataset.df.shape[0])
        ...
        for img,mask in self.test_loader:
            self.logger.info(f'Image Shape = {img.shape}')
            assert(img.shape == (self.params['batch_size'][2],3,256,1600))
            self.logger.info(f'Mask Shape = {mask.shape}')
            assert(mask.shape == (self.params['batch_size'][2],256,1600,self.params['num_class']))
            break
#        itr = 0
